            ...
            <div class="col-md-6">
                <div class="client_box row">
                    <div class="col-md">
                        ...
                        @if ($checkoutorders != null)
                            <div class="client_detail">
                                <span class="h4 text-decoration-underline">Orders History</span>
                                @php
                                    $status = '';
                                    foreach ($checkoutorders as $order) {
                                        ...
                                        echo '<div class="d-flex gap-2 ">' .
                                            ...
                                            $order->product->name .
                                            ':</span>' .
                                            "  <span class='mt-2 badge text-bg-$class'> $status </span></div>";

                                        if ($order->status == 1 or $order->status == 2 ) {
                                            echo " <a class='btn btn-danger btn-sm mt-2' href='/rejectorders/$order->id'>Cancle</a>
                                        ";
                                        }
                                    }
                ...
                <div class="col-6">
                    <div class="ms-4 mt-2">
                        <button class="btn btn-sm border  btn-outline-dark" onclick="address()">Add
                            Address</button>
                    ...
                    <tbody>
                        @foreach ($useraddress as $key => $item)
                            <tr class="{{ $item->default == ' 1' ? 'table-success' : '' }}">
                                ...
                                <td class="d-flex gap-2 "><button onclick="editaddress({{ $item->id }})"
                                        class="btn btn-warning btn-sm" data-toggle="modal"
                                        data-target="#popupModal">Edit</button>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            <div class="form-floating mb-3 ">
                                                ...
                                                <input class="form-control" name="villagename" id="inputFirstNamepop"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-floating mb-3 ">
                                                <select onchange="usercountryof(this.value)" name="usercountry"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-floating mb-3 ">
                                                <select onchange="userstateof(this.value)" name="userstates" id="userstates"
                                                    class="form-select" aria-label="Default select example">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-floating mb-3 ">
                                                <select name="userdistict" id="userdisticts" class="form-select"
                                        ...
                                        <div class="ms-3 form-check">
                                            <input {{ $item->default == ' 1' ? 'checked' : '' }} type="checkbox"
                                                name="default" class="form-check-input" id="exampleCheck1">
                                            <label class="form-check-label" for="exampleCheck1">Make as default</label>
                                        </div>
                                    </div>
                                    <div class="mt-4 mb-3">
                                        <div class="d-grid"><button type="submit" class="btn"
                                                style="background-color: #F6F1ED">Update
                                                Address</button>
                                        </div>
                                    </div>
                                </form>
            ...
            <div id="address" style="display: none" class="col-md-6">
                <form action="{{ route('userprofile') }}" method="POST">
                    ...
                    <div class="row ">
                        ...
                        <div class="col-6">
                            <div class="form-floating mb-3 ">
                                <select onchange="stateof(this.value)" name="state" id="states" class="form-select"
                                    aria-label="Default select example">
                                    <option selected>Select State</option>
                                </select>
                                @if ($errors->has('state'))
                        ...
                        <div class="col-6">
                            <div class="form-floating mb-3 ">
                                <select name="distict" id="disticts" class="form-select"
                                    aria-label="Default select example">
                                    <option selected>Select Distirct</option>
                                </select>
                                ...
                                @foreach ($cartitems as $item)
                                    <tr>
                                        <td class="col-sm-8 col-md-6">
                                            <div class="media"> <a class="thumbnail pull-left"
                                                    href="{{ route('product', $item->usercart->id) }}"> <img
                                                        style="width: 5rem" class=""
                                                        src="{{ $item->usercart->image }}"
                                                        alt="website template image"></a>
                                                <div class="media-body ms-3">
                                                    <a class="h4"
                                                        href="{{ route('product', $item->usercart->id) }}">{{ $item->usercart->name }}</a><br>
                                                    @if ($item->usercart->quantity == 1)
                                                        <span>Status:</span><span class="text-danger">Only 1 left</span>
                                                    @endif
                                                    @if ($item->usercart->quantity == 0)
                                                        <span>Status:</span><span class="text-danger">last item</span>
                                                    @endif
                                                    @if ($item->usercart->quantity > 1)
                                        ...
                                        <td class="col-sm-1 col-md-1" style="text-align: center">
                                            <div class="d-flex align-items-center gap-2">
                                                <div><button onclick="increment({{ $item->usercart->id }})"
                                                        class="btn"><i class="bi bi-plus-lg"></i></button></div>
                                                <button class="btn" id="quantity"
                                                    disabled>{{ $item->quantity }}</button>

                                                <div><button onclick="decrement({{ $item->usercart->id }})"
                                                        class="btn"><i class="bi bi-dash-lg"></i></button></div>
                                            </div>

                                        </td>
                                        <td class="col-sm-1 col-md-1 text-center">
                                            <p id="price" class="price_table" hidden>{{ $item->usercart->price }}
                                            </p>
                                        ...
                                        <td class="col-sm-1 col-md-1 text-center">
                                            {{-- {{$item->usercart->id}} --}}
                                            <p id="total" class="price_table">
                                                ₹{{ \App\Models\UserCart::subtotal($item->usercart->id) }} .00</p>
                                        </td>
                                        <td class="col-sm-1 col-md-1">
                                            <a href="{{ route('removecartitem', $item->usercart->id) }}"
                                                class="btn btn-danger text-white mx-4">Remove
                                            </a>
                                        </td>
                                    </tr>
                                @endforeach
